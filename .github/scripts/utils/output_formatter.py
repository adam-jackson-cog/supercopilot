#!/usr/bin/env python3
"""
Output formatting utilities for SuperCopilot scriptable workflows.
...
class ResultFormatter:
    ...
    def create_finding(finding_id: str,
        ...
        return Finding(finding_id, title, description, severity_enum,
                      file_path, line_number, recommendation, evidence)
    ...
    def format_console_output(result: AnalysisResult) -> str:
        ...
        lines = []
        lines.append(f"=== {result.analysis_type.value.upper()} ANALYSIS: {result.script_name} ===")
