...
class ComplexityAnalyzer:
    ...
    def analyze_cyclomatic_complexity(self, content: str, file_path: Path) -> List[Dict[str, Any]]:
        """Calculate cyclomatic complexity indicators."""
        findings = []
        lines = content.split('\n')
        
        # Count complexity indicators per function
        complexity_keywords = r'\b(?:if|else|elif|for|while|catch|case|switch|&&|\|\|)\b'
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for pattern_name, pattern_info in self.complexity_patterns.items():
                    findings.extend(self._find_pattern_matches(
                        content, lines, file_path, pattern_name, pattern_info, "Complexity"
                    ))
    ...
    def analyze(self, target_path: str) -> 'AnalysisResult':
        ...
        result = ResultFormatter.create_code_quality_result("complexity_metrics.py", target_path)
        ...
        try:
            ...
            result.metadata = {
                ...
                "patterns_checked": len(self.complexity_patterns) + len(self.code_smell_patterns) + len(self.naming_patterns),
                "categories": {
                    "complexity": len([f for f in findings if f['category'] == 'Complexity']),
                    "code_smells": len([f for f in findings if f['category'] == 'Code Smell']),
