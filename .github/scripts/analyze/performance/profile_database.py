...
class DatabaseProfiler:
    ...
    def __init__(self):
        ...
        self.skip_patterns = {
            ...
            'coverage', '.nyc_output', 'target', 'vendor', 'migrations'
        }
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for pattern_name, pattern_info in self.n_plus_one_patterns.items():
                    findings.extend(self._find_pattern_matches(
                        content, lines, file_path, pattern_name, pattern_info, "N+1 Query"
                    ))
                
                # Check slow query patterns
                for pattern_name, pattern_info in self.slow_query_patterns.items():
                    findings.extend(self._find_pattern_matches(
                        content, lines, file_path, pattern_name, pattern_info, "Slow Query"
                    ))
                
                # Check ORM patterns
                for pattern_name, pattern_info in self.orm_patterns.items():
                    findings.extend(self._find_pattern_matches(
                        content, lines, file_path, pattern_name, pattern_info, "ORM Usage"
                    ))
    ...
    def _find_pattern_matches(self, content: str, lines: List[str], file_path: Path,
                            pattern_name: str, pattern_info: Dict[str, str], 
    ...
    def analyze(self, target_path: str) -> 'AnalysisResult':
        ...
        result = ResultFormatter.create_performance_result("profile_database.py", target_path)
        ...
        try:
            ...
            result.metadata = {
                ...
                "patterns_checked": len(self.n_plus_one_patterns) + len(self.slow_query_patterns) + len(self.orm_patterns),
                "categories": {
                    "n_plus_one": len([f for f in findings if f['category'] == 'N+1 Query']),
                    "slow_query": len([f for f in findings if f['category'] == 'Slow Query']),
                    "orm_usage": len([f for f in findings if f['category'] == 'ORM Usage'])
                }
            }
    ...
    def _get_recommendation(self, pattern_type: str) -> str:
        ...
        return recommendations.get(pattern_type, "Review query performance and optimization opportunities")

def main():
    """Main function for command-line usage."""
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python profile_database.py <target_path> [--summary]")
    ...
    result = profiler.analyze(target_path)
