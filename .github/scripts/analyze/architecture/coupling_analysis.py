...
class CouplingAnalyzer:
                ...
                for match in matches:
                    for group_idx in groups:
                        if group_idx <= len(match.groups()) and match.group(group_idx):
                            ...
                            if dep and not dep.startswith('.'):  # Skip relative imports for now
    ...
    def analyze(self, target_path: str) -> 'AnalysisResult':
        ...
        result = ResultFormatter.create_architecture_result("coupling_analysis.py", target_path)
        ...
        try:
            ...
            result.metadata = {
                **graph_stats,
                ...
                "circular_dependencies": len([f for f in coupling_findings if f['pattern_type'] == 'circular_dependency']),
                "high_coupling_modules": len([f for f in coupling_findings if 'fan_' in f['pattern_type']])
            }
    ...
    def _get_recommendation(self, pattern_type: str) -> str:
        ...
        return recommendations.get(pattern_type, "Review architecture for coupling reduction opportunities")

def main():
    """Main function for command-line usage."""
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python coupling_analysis.py <target_path> [--summary]")
